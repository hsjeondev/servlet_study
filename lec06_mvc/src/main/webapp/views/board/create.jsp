		...
		<div id="section_wrap">
			...
			<div class="create_board_form">
				<form action='/boardCreateEnd' name="create_board_form" method="post" enctype="multipart/form-data">	
					...
					<input type="hidden" name="board_writer" value="<%=member.getMemberNo()%>">
		...
		const createBoardForm = function() {
			...
			if(!form.board_title.value){
				...
				if(type == 'jpg' || type == 'png' || type == 'jpeg') {
					// form.submit();
					const sendData = new FormData(form); // form 안에 들어 있는 데이터를 객체화 시켜줌
					$.ajax({
						url : '/boardCreateEnd',
						type : 'post',
						enctype : 'multipart/form-data',
						cache : false, // enctype과 짝꿍
						async : false, // enctype과 짝꿍
						contentType : false, // enctype과 짝꿍
						processData : false, // enctype과 짝꿍, 데이터를 잘라서 보내는 것을 막음
						data : sendData,
						dataType : 'json',
